	...
	public class Scraper {
		...
		private void addSlot(HtmlElement e, Section c, boolean secondRow) {
			...
			for (int j = 0; j < times[0].length(); j+=2) {
				...
				if (Slot.DAYS_MAP.get(code) == null) // For example, if the day is a Sunday
					break;
				
				// The next set of code parses, converts and compares the timings requirements of the scraped section 
		...
		public List<SFQ> scrape(String baseurl, boolean instructor)
		{
			try	{
				
				HtmlPage page = client.getPage(baseurl);
				Vector<SFQ> result = new Vector<SFQ>();
				List<?> departments = (List<?>) page.getByXPath(".//table"); // extracting department tables			
				for (int i=0; i<departments.size(); i++) // traversing departments
				{
					if (instructor == true)	{
						HtmlElement htmlItem = (HtmlElement) departments.get(i);
						
						List<?> subjects = (List<?>) htmlItem.getByXPath(".//tr");
					}
				}
				...
				for (int i = 0; i < items.size(); i++) {
					...
					for ( HtmlElement e: (List<HtmlElement>)sections) {
						...
						if(type[0].startsWith("L") || type[0].startsWith("T"))	{
							...
							s.setType(type[0]);
							s.setsID(type[1]);
							addSlot(e, s, false); s.increaseslotby1();
							e = (HtmlElement)e.getNextSibling();
							if (e != null && !e.getAttribute("class").contains("newsect"))
								addSlot(e, s, true); s.increaseslotby1();
							listsections.add(s);
							}
					}
